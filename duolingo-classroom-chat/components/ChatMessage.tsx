  ...
  return (
    <div className={`p-3 rounded-xl mb-2 ${message.isFlagged ? 'bg-red-100 border border-red-300' : 'bg-white border border-gray-200'}`}>
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs text-gray-400 bg-green-100 px-2 py-0.5 rounded-full">{message.courseId}</span>
      </div>
      <p className="text-gray-800">{message.content}</p>
      {message.isFlagged && <p className="text-xs text-red-500 mt-1">Flagged for review</p>}
    </div>
  );
}
