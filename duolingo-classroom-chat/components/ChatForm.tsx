  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="bg-white rounded-2xl p-4 shadow border border-gray-200">
      <div className="mb-3">
        ...
        <select {...register('courseId')} className="w-full border border-gray-300 rounded-lg p-2 text-sm">
          {COURSES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
      </div>
      <div className="mb-3">
        <label className="block text-sm font-semibold text-gray-600 mb-1">Message</label>
        <textarea {...register('content')} placeholder="Type your message... (try 'spam' to test filtering)" className="w-full border border-gray-300 rounded-lg p-2 text-sm resize-none" rows={3}/>
        {errors.content && <p className="text-red-500 text-xs mt-1">{errors.content.message}</p>}
      </div>
      <input type="hidden" {...register('authorId')} />
      <button type="submit" className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-xl">Send Message 🦜</button>
    </form>
  );
}
